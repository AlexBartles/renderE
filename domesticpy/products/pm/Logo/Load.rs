...
# for the Logo -- so they are each mutually exclusive. Yes, you
# can have a bulletin playing at the same time as the National
# LDL (and/or you can have a bulletin over squeezeback), but
# you'll "never" have this Logo load called with more than one
# of these flags set at the same time.
if nationalLdl:
    ldepth = ldepth - 20
if bulletinActive:
    ldepth = ldepth + 20
...
twccommon.Log.info('loading Logo %s render-script' % (lname))
